            ...
            if (it!=null){
                when(it){
                  is  ResponseResult.Success->{
                      Log.e(TAG, "onViewCreated:oo ${it.data}", )
                      Log.e(TAG, "onViewCreated:oo ${it.data.base_code}", )
                      Log.e(TAG, "onViewCreated:oo ${it.data}", )

                      editor.putString(Constants.CURRENCY_CODE, it.data.base_code)
                      editor.putString(Constants.CURRENCY_VALUE, it.data.toString())
                  }
                  is  ResponseResult.Error->{
                      Log.e(TAG, "onViewCreated:88 ${it.message}", )
                  }
                }
            }
        }
